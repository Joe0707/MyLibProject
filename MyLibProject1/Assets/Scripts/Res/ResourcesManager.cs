...
public class ResourcesManager : Singleton<ResourcesManager>
{
    ...
    public UnityEngine.Object[] LoadAllResource(string path, System.Type type)
    {
        if (string.IsNullOrEmpty(path))
        {
            return null;
        ...
        if (!m_LoadFormAssetBundle)
        {
            ...
            result = LoadAllAssetFromResources(path, type);
        }
        else
        {
    ...
    {
        var array = LoadAllResource(path, typeof(T));
        return System.Array.ConvertAll<UnityEngine.Object, T>(array, new System.Converter<UnityEngine.Object, T>((item) => { return item as T; }));
    }
    //加载AB包的场景
    ...
    public ResourceRequest AsyncLoadAssetFromResources(string path, System.Type type)
    {
    ...
    public UnityEngine.Object[] LoadAllAssetFromResources(string path, System.Type type)
    {
    ...
    /// </summary>
    public void AsyncLoadResource(string path, System.Type type, OnAsyncObjFinish dealFinish, LoadResPriority priority = LoadResPriority.RES_SLOW, object param1 = null, uint crc = 0)
    {
        if (crc == 0)
        {
        ...
        if (!m_LoadingAssetDic.TryGetValue(crc, out para) || para == null)
        {
            ...
            para.m_Crc = crc;
            para.m_Path = path;
            // para.m_Sprite = (type == typeof(UnityEngine.Sprite));
        ...
        while (true)
        {
            ...
            for (int i = 0; i < (int)LoadResPriority.RES_NUM; i++)
            {
                ...
                if (!m_LoadFormAssetBundle)
                {
                    ...
                    // else
                    // {
                    var request = AsyncLoadAssetFromResources(loadingItem.m_Path, loadingItem.m_Type);
                    yield return request;
                    obj = request.asset;
                    // }
                    item = new ResouceItem();
